...
local function getLongLongAlias()
	local rootNodes = getRootNodes()
	for _, v in pairs(rootNodes) do
		if v.node:type() ~= "alias_declaration" then
			goto continueLL
		end
		local nodes = getAllNodes(v.node)
		...
		if
			nodes[4].type == "type_descriptor"
				...
				and nodes[4].children[2].type == "long"
				and nodes[4].children[3].type == "primitive_type"
			or nodes[4].type == "type_descriptor"
				and nodes[4].children[1].type == "long"
				and nodes[4].children[2].type == "long"
		then
			goto continueLL
...
local getLongLong = function()
	...
	if typedef ~= "long long int" then
		return typedef
	end
	local alias = getLongLongAlias()
	if alias ~= "long long int" then
		return alias
	end
	return "long long int"
end
